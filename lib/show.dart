...
class _Fluent2DemoAppState extends State<Fluent2DemoApp> {
  ...
  Widget build(BuildContext context) {
    return FluentTheme(
      themeData: _isDarkMode ? FluentThemeData.dark() : FluentThemeData.light(),
      child: Builder(
        builder: (context) {
...
class _ShowcaseHomePageState extends State<ShowcaseHomePage> {
  ...
  // 根据选中的页码构建分类组件表
  Widget _buildPageCategory(FluentThemeData theme) {
  ...
  // 1. 按钮与选框 Page
  Widget _buildButtonsAndTogglesPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
...
FluentCitation(
  ...
  // 2. 列表与卡片 Page
  Widget _buildListAndCardPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
        ...
        ComponentDocCard(
          ...
          demo: Column(
            children: [
              ...
              const SizedBox(height: 12.0),
              FluentFileCard(
                title: 'Fluent2_Design_Tokens.pdf',
                subtitle: '4.2 MB · 微软设计规范文档',
                onActionTap: () => _log('点击了文件卡片更多选项'),
                onTap: () => _log('点击了文件卡片'),
              ),
              const SizedBox(height: 12.0),
              FluentAnnouncementCard(
                ...
                buttonText: '了解更多',
                onActionTap: () => _log('点击了 AnnouncementCard 操作按钮'),
              ),
            ],
          ),
...
FluentFileCard(
  ...
  // 3. 导航工具栏 Page
  Widget _buildNavigationPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
          ...
          description:
              '微软 Tokenized 顶部导航与 AppBarLayout 容器，支持 Avatar / BackIcon 导航指示、自定义背景/前景色与高对比度对比度自适应、Actions 按钮间距配置与居中标题。',
          demo: Column(
            children: [
              FluentAppBarLayout(
              ...
              FluentTopAppBar(
                ...
                actionsSpacing: 12.0,
                centerTitle: true,
                leftActions: [const Icon(Icons.menu)],
                rightActions: [
                  IconButton(
  ...
  rightActions: [ ... ],
          ...
          params: const [
            ...
            ApiParam(
              ...
              type: 'Color?',
              defaultVal: 'null',
              desc: '自定义背景颜色 (未指定时根据 style 与 FluentTheme 自适应)',
            ),
            ApiParam(
              name: 'foregroundColor / secondaryForegroundColor',
              type: 'Color?',
              defaultVal: 'null',
              desc: '自定义前景/副标题颜色 (未指定时按背景亮度自适应计算高对比度颜色)',
            ),
            ApiParam(
              name: 'actionsSpacing',
              type: 'double',
              defaultVal: '4.0',
              desc: '动作图标与按钮组项之间的水平间距',
            ),
            ApiParam(
              name: 'centerTitle',
              type: 'bool',
              defaultVal: 'false',
              desc: '是否将标题与副标题居中对齐显示',
            ),
            ApiParam(
              name: 'navigationIconType',
              type: 'FluentNavigationIconType',
              defaultVal: 'backIcon',
              desc: 'none, avatar, backIcon 导航图标形态 (AppBarLayout)',
            ),
            ApiParam(
              name: 'accessoryView',
              type: 'Widget?',
...
FluentPillButton(
  ...
  // 4. 文本搜索框 Page
  Widget _buildInputsPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
...
FluentTextField.number(
  ...
  // 5. 模态与通知 Page
  Widget _buildModalsAndNotificationsPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
...
showFluentSnackbarToast(
  ...
  // 6. 人员与微光 Page
  Widget _buildPersonasAndTransientsPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
...
FluentAcrylic(
  ...
  // 7. 日历与图标 Page
  Widget _buildCalendarAndIconsPage(FluentThemeData theme) {
    return Column(
      children: [
        ComponentDocCard(
          theme: theme,
